...
public interface GameExecutionRepository extends JpaRepository<GameExecution, UUID> {
    ...
    @Query("""
            ...
            where e.patientPath.patient.analyst.id = :analystId
              and (:patientId is null or e.patientPath.patient.id = :patientId)
    ...
    @Query("""
            select e from GameExecution e
            where e.id = :id and e.patientPath.patient.analyst.id = :analystId
            """)
    ...
    @Query("""
            ...
            where e.patientPath.uniqueCode = :uniqueCode
              and e.patientPath.patient.analyst.id = :analystId
            order by e.startedAt desc
            """)
